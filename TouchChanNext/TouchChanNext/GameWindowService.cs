...
static partial class ServiceLocator
{
    ...
    public static void InitializeWindowHandle(HWND newHandle) => _current = new(newHandle); // Log
...
class GameWindowService(HWND handle)
{
    public HWND Handle { get; } = handle;

    public double DpiScale => HwndExtensions.GetDpiForWindow(Handle) / 96d;
    ...
    public Observable<Size> ClientSizeChanged() =>
        ...
        {
            PInvoke.GetClientRect(Handle, out var initRect);
            ...
            var targetThreadId = PInvoke.GetWindowThreadProcessId(Handle, out var processId);
